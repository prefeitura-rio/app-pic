...
import { Individual, DashboardSummary } from "@/app/types";
...
export function DashboardClient() {
  ...
  useEffect(() => {
    ...
    if (session?.accessToken) {
      ...
      const fetchDashboard = apiService.getDashboardMetrics(session.accessToken as string)
        .then(data => setDashboardData(data))
        .catch(err => console.error("Failed to load dashboard summary", err));

      // Fetch initial participants for Professional Tab (first page)
      const fetchParticipants = apiService.getParticipants(1, 100, session.accessToken as string) // Fetch slightly more for initial list
        .then(res => setParticipantData(res.data))
        .catch(err => console.error("Failed to load participants", err));

      Promise.all([fetchDashboard, fetchParticipants])
