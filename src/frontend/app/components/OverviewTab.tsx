		...
		return (
			<div className="flex flex-col items-center justify-center py-16 text-center">
				...
				<p className="text-sm text-muted-foreground mt-2 max-w-sm">
					Agregando métricas por secretaria, safra e protocolo. Isso pode levar
					alguns segundos na primeira abertura.
				</p>
			</div>
		);
	}
		...
		<div className="space-y-8">
			...
			<div className="space-y-2">
				...
				<p className="text-sm text-muted-foreground">
					Visão macro para lideranças das secretarias (Saúde, Educação,
					Assistência e Casa Civil)
				</p>
			</div>
			...
			<div className="space-y-6">
				...
				data.tempo_medio_irregularidade.length > 0 ? (
						...
						{data.tempo_medio_irregularidade
							...
							.map((item) => {
								const iconMap: Record<
									string,
									{ icon: typeof Activity; color: string; bgClass: string }
								> = {
									geral: {
										icon: Activity,
										color: "text-primary",
								...
								const {
									...
									color,
									bgClass,
								} = iconMap[item.secretaria] || iconMap.geral;
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
					...
					<Card className="relative">
						...
						<CardHeader>
							<CardTitle className="text-lg">
								Distribuição por Tempo de Irregularidade
							</CardTitle>
							<CardDescription>
						...
						<CardContent>
							{data.distribuicao_tempo_irregularidade &&
							data.distribuicao_tempo_irregularidade.length > 0 ? (
												...
												if (active && payload && payload.length) {
													...
													return (
														<div className="bg-background border rounded-lg p-3 shadow-lg">
															...
															<p className="text-sm">
																Participantes: {d.count.toLocaleString("pt-BR")}
															</p>
															<p className="text-sm">
								...
								<div className="flex items-center justify-center h-[350px] text-muted-foreground">
									<div className="text-center">
										...
										<p className="text-sm mt-2">
											Nenhum dado de tempo de irregularidade disponível
										</p>
									</div>
								</div>
							)}
					...
					<Card className="relative">
						...
						<CardContent>
							{data.taxa_resolucao_mensal &&
							data.taxa_resolucao_mensal.length > 0 ? (
								<ResponsiveContainer width="100%" height={350}>
									<LineChart
														...
														<div className="bg-background border rounded-lg p-3 shadow-lg">
															...
															{showSMAS && (
																<p
																	...
																	style={{ color: "#10b981" }}
																>
																	Assistência: {d.assistencia.toFixed(1)}%
																</p>
															)}
															{showSME && (
																<p
								...
								<div className="flex items-center justify-center h-[350px] text-muted-foreground">
									<div className="text-center">
										...
										<p className="text-lg font-medium">Sem dados</p>
										<p className="text-sm mt-2">
											Nenhum dado de taxa de resolução disponível
										</p>
									</div>
								</div>
							)}
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<Card className="relative">
					...
					<CardHeader>
						<CardTitle className="text-xl">
							Participantes por Mês de Ingresso no Programa
						</CardTitle>
						<CardDescription>
							Acompanhamento de entrada e saída de participantes do programa ao
							longo do tempo
						</CardDescription>
					</CardHeader>
					<CardContent>
						{data.distribuicao_por_safra &&
						data.distribuicao_por_safra.length > 0 ? (
										...
										content={({ active, payload }) => {
											if (active && payload && payload.length) {
												...
												return (
													<div className="bg-background border rounded-lg p-3 shadow-lg">
														<p className="font-semibold mb-2">
															Mês de Ingresso: {d.safra}
														</p>
														<p className="text-sm" style={{ color: "#3b82f6" }}>
															Ativos:{" "}
															{(d.total_ativos || 0).toLocaleString("pt-BR")}
														</p>
														<p className="text-sm" style={{ color: "#ef4444" }}>
															Inativos:{" "}
															{(d.total_inativos || 0).toLocaleString("pt-BR")}
														</p>
														<p className="text-sm text-muted-foreground mt-1">
															Total:{" "}
															{(d.total_participantes || 0).toLocaleString(
																"pt-BR",
															)}
														</p>
													</div>
							...
							<div className="flex items-center justify-center h-[450px] text-muted-foreground">
								<div className="text-center">
									...
									<p className="text-sm mt-2">
										Gráfico de barras: participantes ativos/inativos por safra
									</p>
								</div>
							</div>
						)}
				...
				<Card className="relative">
					...
					<CardHeader>
						...
						<CardDescription>
							Distribuição dos motivos pelos quais participantes saíram do
							programa
						</CardDescription>
					</CardHeader>
					<CardContent>
						{data.distribuicao_motivo_saida &&
						data.distribuicao_motivo_saida.length > 0 ? (
							<div className="space-y-4">
								<ResponsiveContainer width="100%" height={380}>
									<PieChart>
										<Pie
											data={
												data.distribuicao_motivo_saida as unknown as Record<
													string,
													unknown
												>[]
											}
											...
											label={({
												...
												const radius =
													percent < 0.05 ? outerRadius + 45 : outerRadius + 30;
										...
										<Tooltip
											content={({ active, payload }) => {
												if (active && payload && payload.length) {
													const d = payload[0].payload;
													const total = data.distribuicao_motivo_saida.reduce(
														(acc, item) => acc + (item.total || 0),
														0,
													);
													const percent =
														total > 0
															? (((d.total || 0) / total) * 100).toFixed(1)
															: 0;
													return (
														<div className="bg-background border rounded-lg p-3 shadow-lg">
							...
							<div className="flex items-center justify-center h-[450px] text-muted-foreground">
								<div className="text-center">
									...
									<p className="text-sm mt-2">
										Nenhum participante inativo no período selecionado
									</p>
								</div>
							</div>
						)}
